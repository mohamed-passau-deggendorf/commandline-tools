...
&& (pid=pidstr(name,size))>0 && pid !=cpid
			...
			int pfd =  openat(fd,name, O_RDONLY | O_DIRECTORY);
				...
				char exe_buffer[2048];
			 	...
			 	char cmdline_buffer[2048];
                        size_t cmdline_size = readlinkat(pfd,"cmdline",cmdline_buffer,2048);
                        cmdline_buffer[cmdline_size]=0;
			


if(argc< 2 || strcmp(cmdline_buffer,argv[1]) == 0 || strcmp(exe_buffer,argv[1]) == 0 || strcmp(name,argv[1])) {
printf("%s %d %s\n",cmdline_buffer,pid,exe_buffer);
